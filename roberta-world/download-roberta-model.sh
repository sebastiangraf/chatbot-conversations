...
if [[ ! -s "${MODELS_TARGET_FOLDER}/${DOWNLOAD_FILENAME}" ]]; then
    ...
    echo "Warning: ${DOWNLOAD_FILENAME} already exists, proceeding with unzipping. Delete the file to download it again."
fi

tar -C ${MODELS_TARGET_FOLDER} -zxvf "${MODELS_TARGET_FOLDER}/${DOWNLOAD_FILENAME}"
echo "Finished unpacking ${DOWNLOAD_FILENAME} in the ${MODELS_TARGET_FOLDER} folder."
